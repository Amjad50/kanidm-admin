...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/people/{id}/credentials/reset",
            get(credentials::reset_modal).post(credentials::submit_reset),
        )
        .route("/people/{id}/ssh", get(ssh::tab).post(ssh::add))
        .route(
            "/people/{id}/ssh/{tag}/delete",
            axum::routing::post(ssh::delete),
        )
        .route("/people/{id}/radius", get(radius::tab))
        .route(
            "/people/{id}/radius/regenerate",
            axum::routing::post(radius::regenerate),
        )
        .route(
            "/people/{id}/radius/delete",
            axum::routing::post(radius::delete_secret),
        )
        ...
        .route(
            "/people/{id}/sessions/{session_id}/destroy",
            axum::routing::post(sessions::destroy_one),
        )
        .route(
            "/people/{id}/sessions/destroy_all",
            axum::routing::post(sessions::destroy_all),
        )
        .route("/people/{id}/groups", get(groups_tab::tab))
        .route(
            "/people/{id}/groups/add",
            axum::routing::post(groups_tab::add),
        )
        .route("/people/{id}/validity", get(validity::tab))
        .route(
            "/people/{id}/validity/valid_from",
            axum::routing::post(validity::set_valid_from),
        )
        .route(
            "/people/{id}/validity/expire",
            axum::routing::post(validity::set_expire),
        )
