...
| 6 | Toast notification (success / info / warn / error) | extracted | Preact island bound to `#toast-stack` + server-side `Toast` struct → `HX-Trigger` header | `islands/toast.tsx` (client) + `src/views/toast.rs` (server `Toast` + `Toast::hx_trigger()`) | 5.4 (people SSH add); any mutation handler can attach via `resp.headers_mut().insert("HX-Trigger", toast.hx_trigger())` | 5.4 |
...
| 3 | Toast renderer | extracted | `islands/toast.tsx` (Preact island bound to `#toast-stack`); mounted by `mountToasts()` in `islands/entry.ts` | DOM event named `toast` on `document.body` (fired by HTMX from `HX-Trigger` header); also `window.showToast({title, kind, desc})` |
